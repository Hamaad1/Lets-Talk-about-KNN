...
results_directory = os.path.join(results_directory, 'Results and analysis', 'Results_pos_err', 'knn_plain2024', 'C4')
...
strategies = ['unweighted', 'inverse_distance', 'squared_inverse_distance']
alpha = 0.0001
...
for base_name in ['DSI1', 'DSI2', 'LIB1', 'LIB2', 'MAN1', 'MAN2', 'SAH1', 'SIM001', 'TIE1', 'TUT1','TUT2', 'TUT3', 'TUT4', 'TUT5', 'TUT6', 'TUT7', 'UJI1', 'UTS1', 'GPR00' , 'GPR01' , 'GPR02', 'GPR03', 'GPR04', 'GPR05', 'GPR06', 'GPR07', 'GPR08', 'GPR09', 'GPR10', 'GPR11', 'GPR12', 'GPR13' ,'SOD01', 'SOD02', 'SOD03', 'SOD04', 'SOD05', 'SOD06', 'SOD07', 'SOD08', 'SOD09', 'UEXB1', 'UEXB2', 'UEXB3', 'UJIB1', 'UJIB2']:  # Add more base names as needed
    ...
    if defaultNonDetectedValue != 0: #removed .size our if len(defaultNonDetectedValue) != 0: for list 
        database0 = replace_non_detected_values(database0, defaultNonDetectedValue, newNonDetectedValue)#, initial_trnrss_values, initial_tstrss_values, trnrss_mask, tstrss_mask
        ...
        for distance_metric in distance_metrics:
            for strategy in strategies:
                ...
                for i in range(test_rssi.shape[0]):
                    ...
                    while n_candidates < len(distancessort) - 1:#rsamplesreduced:
                ...
                # Step 7: Remap buildings floors to real-world values
                ...
                mean_errors_list.append({'Dataset Name': base_name, 'k': k, 'Strategy': strategy, 'Distance Metric': distance_metric, 'Mean Error': np.round(mean_error, 3)})
                ...
                #print(f'{base_name} Test Mean 3D Positioning Error: {mean_error}')
                ...
                error_df = pd.DataFrame({'Error': np.round(test_errors, 2)})  # Round errors to 4 decimal places
                ...
                # Update best configuration for each dataset
                if base_name not in best_configs_dict or best_configs_dict[base_name]['Mean Error'] > mean_error:
                    best_configs_dict[base_name] = {
                        'k': k,
                        'Strategy': strategy,
                        'Distance Metric': distance_metric,
                        'Mean Error': np.round(mean_error, 2)
                    }
                ...
                # plt.figure(figsize=(10, 8))
                # plt.quiver(test_df_coord_cleaned['Longitude'], test_df_coord_cleaned['Latitude'],
                #            y_test_pred[:, 1] - test_df_coord_cleaned['Longitude'],
                #            y_test_pred[:, 0] - test_df_coord_cleaned['Latitude'],
...
# Create a DataFrame for best configurations
