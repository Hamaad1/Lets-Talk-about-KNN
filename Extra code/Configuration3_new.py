...
def compute_distances(test_samples, train_samples, distance_metric='cityblock', similarity_function=None, alpha=None):
    epsilon = 1e-10  # Define epsilon inside the function for cosine distance
    if similarity_function is None:
        if distance_metric == 'cityblock':
            return np.sum(np.abs(train_samples[:, None, :] - test_samples[None, :, :]), axis=2).T
        elif distance_metric == 'euclidean':
            return np.sqrt(np.sum((train_samples[:, None, :] - test_samples[None, :, :]) ** 2, axis=2)).T
        elif distance_metric == 'minkowski3':
            return np.sum(np.abs(train_samples[:, None, :] - test_samples[None, :, :]) ** 3, axis=2).T ** (1/3)
        elif distance_metric == 'cosine':
            train_magnitude = np.sqrt(np.sum(train_samples ** 2, axis=1))
            test_magnitude = np.sqrt(np.sum(test_samples ** 2, axis=1))
            train_magnitude = np.clip(train_magnitude[:, None], epsilon, None)
            test_magnitude = np.clip(test_magnitude, epsilon, None)
            cosine_similarity = np.dot(train_samples, test_samples.T) / (train_magnitude * test_magnitude)
            return (1 - cosine_similarity).T
        elif distance_metric == 'sorensen':
            denom = np.sum(train_samples[:, None, :] + test_samples[None, :, :], axis=2)
            denom = np.clip(denom, epsilon, None)  # Avoid division by zero
            return (np.sum(np.abs(train_samples[:, None, :] - test_samples[None, :, :]), axis=2) / denom).T
        elif distance_metric == 'neyman':
            factorzero = 0.000001 if np.max(train_samples) <= 1.0001 and np.max(test_samples) <= 1.0001 else 0.0001
            divisor = test_samples + (factorzero * (test_samples == 0))
            return np.sum(((train_samples[:, None, :] - test_samples[None, :, :]) ** 2) / divisor[None, :, :], axis=2).T
            ...
            sigma = 5
            threshold = 0.0001
            numerator = -((train_samples[:, None, :] - test_samples[None, :, :]) ** 2)
            denominator = 2 * (sigma ** 2)
            differences = (1 / np.sqrt(2 * np.pi * sigma ** 2)) * np.exp(numerator / denominator)
            nonzero = (train_samples[:, None, :] * test_samples[None, :, :]) != 0
            return (-np.sum(np.log(differences * nonzero + threshold * (1 - nonzero)), axis=2)).T
        elif distance_metric == 'plgd':
            threshold = alpha  # Assuming 'alpha' is used as the threshold
            p1 = np.sum((train_samples[:, None, :] - threshold) * (train_samples[:, None, :] >= threshold) * (test_samples[None, :, :] == 0), axis=2)
            p2 = np.sum((test_samples[None, :, :] - threshold) * (test_samples[None, :, :] >= threshold) * (train_samples[:, None, :] == 0), axis=2)
            d_lgd = compute_distances(test_samples, train_samples, distance_metric='lgd')
            return d_lgd + (1 / 10) * (p1 + p2)
        else:
            raise ValueError("Unknown distance metric.")
    else:
        similarity_values = similarity_function(test_samples, train_samples)
...
def knn_positioning(train_rssi, train_coords, test_rssi, k, strategy='unweighted', distance_metric='cityblock', alpha=None):
    all_distances = compute_distances(test_rssi, train_rssi, distance_metric=distance_metric, alpha=alpha)
    sorted_indices = np.argsort(all_distances, axis=1)
    
    estimated_positions = []
    for i in range(test_rssi.shape[0]):
        distances = all_distances[i]
        nearest_indices = sorted_indices[i, :k]
        
        # Handle Ties in Distances
        k_distance = distances[nearest_indices[-1]]  # Distance of the k-th nearest neighbor
        additional_indices = np.where(distances == k_distance)[0]
        all_nearest_indices = np.unique(np.concatenate((nearest_indices, additional_indices)))
...
####################################################################
    ...
    newNonDetectedValue = 0  # Default value if not handled specifically

    if newNonDetectedValue is 0:
