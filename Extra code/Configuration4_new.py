...
def knn_positioning11(train_rssi, train_coords, test_rssi, k, strategy='unweighted', distance_metric='cityblock', alpha=None):
    ...
    estimated_positions = np.empty((test_rssi.shape[0], train_coords.shape[1]))
    for i in range(test_rssi.shape[0]):
        ...
        n_candidates = k
        while n_candidates < len(distances) and abs(distances[indices[min(n_candidates, len(indices)-1)]] - distances[indices[n_candidates - 1]]) < 1e-12:
            n_candidates += 1

        all_nearest_indices = indices[:min(n_candidates, len(indices))]
        nearest_positions = train_coords[all_nearest_indices]
        nearest_distances = distances[all_nearest_indices]
        
        estimated_positions[i] = compute_weighted_centroid(nearest_positions, nearest_distances, strategy)
